...
namespace Telemachus.Data.Services.Context
{
    ...
    public class BunkeringValueGenerator : ValueGenerator<string>
    {
        ...
        public override string Next(EntityEntry entry)
        {
            ...
            var lastRecord = context.BunkeringData.Where(_ => _.BusinessId.StartsWith(userPrefix)).OrderBy(_ => _.BusinessId.Length).ThenBy(_ => _.BusinessId).IgnoreQueryFilters().LastOrDefault();
        ...
        public override async ValueTask<string> NextAsync(EntityEntry entry, CancellationToken token = default(CancellationToken))
        {
            ...
            var context = (TelemachusContext)entry.Context;
            var userId = entry.CurrentValues.GetValue<string>("UserId");
            var userPrefix = (await context.Users.Where(_ => _.Id == userId).Select(_ => _.Prefix).SingleAsync()).ToUpper();
            if (userPrefix == null)
            {
                throw new ArgumentNullException(nameof(userPrefix));
            }
            var lastRecord = await context.BunkeringData.Where(_ => _.BusinessId.StartsWith(userPrefix)).OrderBy(_ => _.BusinessId.Length).ThenBy(_ => _.BusinessId).IgnoreQueryFilters().LastOrDefaultAsync();
            var lastId = lastRecord?.BusinessId;
            string id = lastId == null ?
