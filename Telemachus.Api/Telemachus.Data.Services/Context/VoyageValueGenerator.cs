...
namespace Telemachus.Data.Services.Context
{
    ...
    public class VoyageValueGenerator : ValueGenerator<string>
    {
        ...
        public override string Next(EntityEntry entry)
        {
            ...
            var lastRecord = context.Voyages.Where(_ => _.BusinessId.StartsWith(userPrefix)).OrderBy(_ => _.BusinessId.Length).ThenBy(_ => _.BusinessId).IgnoreQueryFilters().LastOrDefault();
        ...
        public override async ValueTask<string> NextAsync(EntityEntry entry, CancellationToken token = default(CancellationToken))
        {
            ...
            var lastRecord = await context.Voyages.Where(_ => _.BusinessId.StartsWith(userPrefix!.ToUpper())).OrderBy(_ => _.BusinessId.Length).ThenBy(_ => _.BusinessId).IgnoreQueryFilters().LastOrDefaultAsync();
            var lastId = lastRecord?.BusinessId;
            string id = lastId == null ?
